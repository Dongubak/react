...
서버사이드 렌더링이란 UI를 서버에서 렌더링 하는 것을 의미한다.

우리가 지금까지 만든 리액트 프로그램은 클라이언트 사이드 렌더링을 하고 있었다. 이는 UI 렌더링을 브라우저에서 모두 처리하는 것이다.

즉 자바스크립트를 브라우저에서 실행해야 사용할 수 있는 우리가 만든 화면이 사용자에서 나오는 것이다.

우리가 npm start를 한 후에는 root가 비어있다. 자바스크립트가 실행되고 리액트 컴포넌트가 렌더링이 된 후에 root 엘리먼트가 보여진다.

서버사이드 렌더링을 구현하면 사용자가 웹 서비스를 방문했을 때 초기렌더링을 서버 쪽에서 대신 해준다. 사용자가 html을 전달 받을 때 그 내부에 렌더링 결과물이 반영되어 있으며 클라이언트에서 렌더링을 줄인다.

#### 서버사이드 렌더링의 장점
1. 크롤링시 검색엔진을 이용하여 페이지를 원활하게 수집이 가능하다
2. 초기 렌더링 성능을 개선할 수 있다.
      클라이언트는 자바스크립트가 실행되어 컴포넌트가 로드되기 전에는 빈화면을 봐야한다. 만약 초기 페이지를 구성할 때 API요청을 한다면 대기시간은 더 늘어날 것이며 자바스크립트 파일 다운로드가 완료되지 않은 시점에도 html상에 사용자가 볼 수 있는 콘텐츠를 볼 수 있으며 사용자 경험도 향상된다.

#### 서버사이드 렌더링의 단점
1. 브라우저가 할일을 서버에서 하므로 서버 리소스가 사용된다.
      갑자기 수많은 사용자가 동시에 웹서비스에 접속시 서버가 과부화될 수 있다.
      따라서 사용자가 많은 경우 캐싱과 로드 밸런싱을 통해 성능을 최적화 해야한다.

2. 서버사이드 렌더링을 하면 프로젝트의 구조가 복잡해지고 데이터를 미리 불러오기, 코드스플리팅과의 호환등 고려할 점이 많아진다.


#### 서버사이드 렌더링과 코드 스플리팅의 충돌
...
서버사이드 렌더링 결과물이 나타난 후 자바스크립트가 로딩되면서 다시 불러오지 않은 컴포넌트를 null로 렌더링하면서 이후에 코드스플리팅된 컴포넌트들을 렌더링하면서 깜박임이 발생한다.
